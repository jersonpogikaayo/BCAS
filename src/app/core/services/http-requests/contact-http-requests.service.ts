...
import { Contact } from '../../models/customer/customer.model';
import { retry, catchError } from 'rxjs/operators';

@Injectable({
  providedIn: 'root'
})
export class ContactHttpRequestsService {
  ...
  getGridData(params: any, forceRefresh: boolean = false): Observable<any> {
    ...
    const url = `${this.baseUrl}Contact/find/`;
  ...
  getGridDataCount(params: any, forceRefresh: boolean = false, isEngineer: boolean = true): Observable<number> {
    ...
    const url = `${this.baseUrl}Contact/find/count/`;
  ...
  addContact(payload: Contact): Observable<Contact> {
    return this.http.post<Contact>(`${this.baseUrl}Contact`, payload)
      .pipe(
        retry(1), // Only retry once for POST requests
      );
    }

    editContact(payload: Contact): Observable<Contact> { 
      return this.http.put<Contact>(`${this.baseUrl}Contact`, payload)
        .pipe(
