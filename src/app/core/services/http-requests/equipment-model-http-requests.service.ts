...
export class EquipmentsModelHttpRequestsService  {
  ...
  getGridData(params: any, forceRefresh: boolean = false): Observable<any> {
    ...
    const url = `${this.baseUrl}equipment/model/find/`;
  ...
  getGridDataCount(params: any, forceRefresh: boolean = false, isEngineer: boolean = true): Observable<number> {
    ...
    const url = `${this.baseUrl}equipment/model/find/count/`;
  ...
  deleteExcelTemplate(excelTemplateId: number) {
        return this.http.delete(this.baseUrl + 'excel/template/' + excelTemplateId, this.getHttpOptions())
        .pipe(map(result => {
            return result;
        }, (error: any) => {
            return error
