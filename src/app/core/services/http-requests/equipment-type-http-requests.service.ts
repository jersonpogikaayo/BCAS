...
import { EquipmentType } from '../../models/equipment/equipment.model';
import { retry } from 'rxjs/operators';

@Injectable({
  providedIn: 'root'
})
export class EquipmentTypeHttpRequestsService {
  ...
  getGridData(params: any, forceRefresh: boolean = false): Observable<any> {
    ...
    const url = `${this.baseUrl}equipment/type/find?`;
  ...
  getGridDataCount(params: any, forceRefresh: boolean = false, isEngineer: boolean = true): Observable<number> {
    ...
    const url = `${this.baseUrl}equipment/type/find/count/`;
    ...
    addEquipmentType(payload: EquipmentType): Observable<EquipmentType> {
        return this.http.post<EquipmentType>(`${this.baseUrl}equipment/type`, payload)
        .pipe(
            retry(1), // Only retry once for POST requests
        );
    }

    editEquipmentType(payload: EquipmentType): Observable<EquipmentType> {
    return this.http.put<EquipmentType>(`${this.baseUrl}equipment/type`, payload)
        .pipe(
            retry(1), // Only retry once for PUT requests
        );
    }
}