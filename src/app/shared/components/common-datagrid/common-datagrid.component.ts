...
@Component({
  selector: 'app-common-datagrid',
  templateUrl: './common-datagrid.component.html',
  styleUrls: ['./common-datagrid.component.scss']
})
export class CommonDatagridComponent implements OnInit, OnDestroy {
  ...
  @Input() columnHeader: ColumnHeaderModel[] = [
    ...
    { prettyName: 'Due Date', technicalName: 'dueDate', visible: true },
    { prettyName: 'Status', technicalName: 'status', visible: true },
    { prettyName: 'Site Name', technicalName: 'siteName', visible: true },
    { prettyName: 'Post Code', technicalName: 'sitePostCode', visible: true },
  ...
  getActionsForStatus(statusType: number): string[] {
    ...
    return status ? this.actionLists[status] || [] : [];
  }

  handleAction(action: string, item: GridItem) {
    // For download actions, the loading will be managed by the download service
    if (action !== 'Download Excel' && action !== 'Download PDF') {
      this.actionLoading[item.id] = true;
    }
  ...
  // Get status display name for selected items
  getSelectedItemsStatusSummary(): string {
    if (this.selectedItems.length === 0) {
      return '';
    }
    ...
    this.selectedItems.forEach(item => {
      const statusName = statusMap[item.jobStatusType] || 'Unknown';
