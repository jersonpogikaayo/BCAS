...
@Component({
  selector: 'app-customers-data-grid',
  templateUrl: './customers-data-grid.component.html',
  styleUrls: ['./customers-data-grid.component.scss']
})
export class CustomersDataGridComponent implements OnInit {
  ...
  @Output() actionSelected = new EventEmitter<{action: string, item: GridItem | null}>();
  ...
  handleAction(action: string, item: GridItem | null) {
    console.log('Action:', action, 'Item:', item);
    if (!item){
      if(action === 'add new customer') {
        this.actionSelected.emit({ action: 'add new customer', item: null });
        return;
      } else {
        this.actionSelected.emit({ action: 'upload from csv', item: null });
        return;
      }
    };
    ...
    if (action === 'edit') {
      ...
      this.actionSelected.emit({ action: 'delete', item });
    } else if (action === 'view child') {
      this.actionSelected.emit({ action: 'view child', item });
    }
  }
