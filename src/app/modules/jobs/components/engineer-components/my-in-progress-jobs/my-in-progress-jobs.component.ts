...
@Component({
  selector: 'app-my-in-progress-jobs',
  templateUrl: './my-in-progress-jobs.component.html',
  styleUrls: ['./my-in-progress-jobs.component.scss']
})
export class MyInProgressJobsComponent implements OnInit {
  ...
  gridParameter = {
    jobStatusType: 7, // In-progress jobs
  }
        ...
        if (action === 'View Detail') {
          ...
          try {
            ...
            if (result && result.success) {
              ...
              } else if(result.message === 'Job start was cannot locate') {
                console.log('📝 Job start was cannot locate');
                const cannotLocateProcess = await this.openCannotLocateModal(job);
              }
            }
          ...
          return;
        } else if(action === 'Pause (Incomplete)') {
          const failProcess = await this.handleManualFailPauseFromJobModal(job, false);
        } else if(action === 'Cannot Locate') {
          await this.openCannotLocateModal([job]);
          return;
        } else {
          Swal.fire({
            icon: 'warning',
          ...
          next: (jobDetails: JobDetail) => {
            ...
            forkJoin(dataRequests).subscribe({
              next: (response: any) => {
                ...
                modalRef.componentInstance.intermediateResult.subscribe(async (result: string) => {
                  console.log('Intermediate result from modal:', result);

                  if (result === 'manual fail') {
                    console.log('📝 Handling manual fail without closing the modal');
                    try {
                      await this.handleManualFailPauseFromJobModal(job);
                    } catch (err) {
                      console.error('Error while handling manual fail:', err);
                    }
                  } else { }
                });
                ...
                modalRef.result.then((result: string) => {
                  ...
                  } else if (result === 'cannot locate') {
                    console.warn('⚠️ Job start was cannot locate');
                    resolve({ success: false, message: 'Job start was cannot locate' });
                  } else  {
